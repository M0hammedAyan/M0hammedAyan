# 👋 Hi, I'm Mohammed Ayan

<p align="center">
  <img src="https://img.shields.io/badge/AI%20%26%20ML-Engineer-blueviolet?style=for-the-badge" height="35">
  <img src="https://img.shields.io/badge/Robotics-Enthusiast-orange?style=for-the-badge" height="35">
  <img src="https://img.shields.io/badge/Computer%20Vision-Focused-green?style=for-the-badge" height="35">
  <img src="https://img.shields.io/badge/Mechanical-Engineer-Maroon?style=for-the-badge" height="35">
</p>

<p align="center">
  🎓 AI & ML Engineering Student | 📍 Bengaluru, India
</p>

---

## 🚀 About Me

I’m an AI/ML engineer in progress with a strong interdisciplinary base in **Mechanical Engineering, Robotics, and Intelligent Systems**.

Currently a **Research Intern at IISc**, working on:

> 🧠 **AI-powered healthcare system for early detection of movement disorders in premature infants**

I focus on building **complete systems**, not just models:
- AI + Computer Vision  
- Robotics + IoT  
- Hardware + Software integration  

---

## 🔬 Current Work @ IISc

<p>
  <img src="https://img.shields.io/badge/Multi--Camera-System-black?style=for-the-badge" height="45">
  <img src="https://img.shields.io/badge/Real--Time-Processing-red?style=for-the-badge" height="45">
  <img src="https://img.shields.io/badge/Secure-Transmission-blue?style=for-the-badge" height="45">
  <img src="https://img.shields.io/badge/PyQt-Interface-green?style=for-the-badge" height="45">
  <img src="https://img.shields.io/badge/Hardware-Design-orange?style=for-the-badge" height="45">
</p>

- 📷 Action Cameras + Intel RealSense  
- ⚡ 30–60 FPS real-time pipeline  
- 🔐 Encrypted video streaming  
- 🖥️ Monitoring dashboard (PyQt)  
- 🔧 Custom-built hardware mounts  

---

## 🧠 Projects

### 📊 Student Attendance & Risk Analysis
<p>
  <img src="https://img.shields.io/badge/Multi--User-System-purple?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/Data--Driven-Prediction-blue?style=for-the-badge" height="40">
</p>

- Multi-role system (Admin, HOD, Mentor, Teacher)  
- Predicts **student failure risk**  
- Built in a team of 4  

---

### 🚗 Driver State Monitoring *(Published Research)*
<p>
  <img src="https://img.shields.io/badge/Computer-Vision-red?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/Multimodal-AI-black?style=for-the-badge" height="40">
</p>

- Fatigue + distraction detection  
- Dual-agent architecture  
- Real-time monitoring system  

---

### 🌊 FloodGuard – Flood Detection
- ML-based prediction using sensor data  
- Telegram-based real-time alerts  

---

### 🤖 CareerOS – AI Profile Assistant
- Resume + LinkedIn analysis tool  
- Resume, JD, and Resume Improvising Tool

---

### ⚙️ Magnetic Yoke (NDT System)
- Defect detection using electromagnetic principles  
- Mechanical + electrical system integration  

---

## 🛠️ Tech Stack

### 💻 Core Skills
<p>
  <img src="https://img.shields.io/badge/Python-blue?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/Machine%20Learning-green?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/Artificial%20Intelligence-purple?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/LLMs-orange?style=for-the-badge" height="40">
</p>

### 👁️ Vision & Robotics
<p>
  <img src="https://img.shields.io/badge/OpenCV-black?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/ROS-blue?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/Gazebo-gray?style=for-the-badge" height="40">
</p>

### 🔌 Systems
<p>
  <img src="https://img.shields.io/badge/Raspberry%20Pi-red?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/IoT-green?style=for-the-badge" height="40">
</p>

### 🧰 Tools
<p>
  <img src="https://img.shields.io/badge/Git%20%26%20GitHub-black?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/PyQt-blue?style=for-the-badge" height="40">
  <img src="https://img.shields.io/badge/SolidWorks-red?style=for-the-badge" height="40">
</p>

---

## 📈 Growth Focus

- Building **deployable AI systems (not just notebooks)**  
- Improving consistency on GitHub  
- Deep learning + robotics integration  

---

## 📫 Connect

<p>
  <a href="mailto:mohammedayan262005@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge" height="50">
  </a>

  <a href="https://www.linkedin.com/in/mohammedayan26">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge" height="50">
  </a>
</p>

---

## ⚡ Fun Fact

> I don’t just build AI models — I build the **entire system from hardware to intelligence** 🚀
